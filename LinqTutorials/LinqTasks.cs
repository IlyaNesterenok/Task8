    ...
    public static class LinqTasks
    {
        ...
        public static IEnumerable<object> Task11()
        {
            return from emp in Emps
                ...
                where g.Count() > 1
                select new { name = Depts.FirstOrDefault(d => d.Deptno == g.Key)?.Dname, numOfEmployees = g.Count() };
        }
        ...
        {
            // Filter employees who have subordinates
            var employeesWithSubordinates = Emps.Where(emp => Emps.Any(subordinate => subordinate.Mgr == emp));

            // Sort the filtered employees by name (ascending) and salary (descending)
            var sortedEmployees = employeesWithSubordinates.OrderBy(emp => emp.Ename).ThenByDescending(emp => emp.Salary);
        ...
        {
            return Depts.Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) == 0 || Emps.Count(emp => emp.Deptno == dept.Deptno) == 5)
